...
This package reconciles Cluster API objects and manages the lifecycles of load balancers using ako-operator.
...
* @TODO(fhan): add instructions of setting up AVI controller on vc7
...
EOF
```
After that, verify that the service has been assigned an external IP using `kubectl` and the AVI controller UI.
```shell
$ kubectl get svc
